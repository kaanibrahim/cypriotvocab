...
function normalizeSearchText(value) {
    return String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/ı/g, 'i').replace(/ς/g, 'σ').replace(/[’']/g, '').replace(/[/\\|_-]+/g, ' ').replace(/[^\p{L}\p{N}]+/gu, ' ').replace(/\s+/g, ' ').trim();
}
...
function transliterateSearchText(value) {
    let normalized = normalizeSearchText(value).replace(/ου/g, 'ou').replace(/αι/g, 'ai').replace(/ει/g, 'ei').replace(/οι/g, 'oi').replace(/αυ/g, 'av').replace(/ευ/g, 'ev').replace(/μπ/g, 'b').replace(/ντ/g, 'd').replace(/γκ/g, 'g').replace(/τσ/g, 'ts').replace(/τζ/g, 'tz');
    normalized = Array.from(normalized).map(char => GREEK_TRANSLITERATION_MAP[char] || char).join('');
...
function searchDistance(a, b) {
    ...
    // Treat one adjacent letter swap as the same kind of typo as one edit.
...
const SEARCH_FIELDS = ['cypriot', 'english', 'turkish', 'greek', 'definition'];
...
const SEARCH_CATEGORY_RULES = {
    places: ['village', 'place name', 'toponym', 'koy', 'yer adi', 'kasaba', 'χωριο', 'τοπωνυμιο', 'ονομα χωριου', 'ονομα τοπου'],
    expressions: ['expression', 'phrase', 'interjection', 'curse', 'insult', 'saying', 'ifade', 'deyim', 'atasozu', 'kufur', 'hitap', 'unlem', 'εκφραση', 'φραση', 'βρισια', 'προσφωνηση', 'επιφωνημα'],
    food: ['food', 'bread', 'cheese', 'meat', 'fish', 'fruit', 'vegetable', 'drink', 'coffee', 'milk', 'wine', 'meal', 'cake', 'sweet', 'almond', 'watermelon', 'cucumber', 'tomato', 'potato', 'olive', 'yemek', 'ekmek', 'peynir', 'et', 'balik', 'meyve', 'sebze', 'icecek', 'kahve', 'sut', 'sarap', 'tatli', 'badem', 'karpuz', 'salatalik', 'domates', 'patates', 'zeytin', 'φαγητο', 'ψωμι', 'τυρι', 'κρεας', 'ψαρι', 'φρουτο', 'λαχανικο', 'ποτο', 'καφες', 'γαλα', 'κρασι', 'γλυκο', 'αμυγδαλο', 'καρπουζι', 'αγγουρι', 'ντοματα', 'πατατα', 'ελιά'],
    nature: ['animal', 'bird', 'insect', 'plant', 'tree', 'flower', 'snake', 'fish', 'weather', 'wind', 'rain', 'sea', 'mountain', 'hayvan', 'kus', 'bocek', 'bitki', 'agac', 'cicek', 'yilan', 'hava', 'ruzgar', 'yagmur', 'deniz', 'dag', 'ζωο', 'πουλι', 'εντομο', 'φυτο', 'δεντρο', 'λουλουδι', 'φιδι', 'καιρος', 'ανεμος', 'βροχη', 'θαλασσα', 'βουνο'],
    people: ['person', 'man', 'woman', 'child', 'family', 'friend', 'worker', 'kisi', 'erkek', 'kadin', 'cocuk', 'aile', 'arkadas', 'isci', 'ανθρωπος', 'αντρας', 'γυναικα', 'παιδι', 'οικογενεια', 'φιλος'],
    objects: ['tool', 'clothing', 'garment', 'shoe', 'chair', 'door', 'house', 'vehicle', 'car', 'bus', 'container', 'fabric', 'alet', 'giysi', 'ayakkabi', 'sandalye', 'kapi', 'ev', 'araba', 'otobus', 'kap', 'kumas', 'εργαλειο', 'ρουχο', 'παπουτσι', 'καρεκλα', 'πορτα', 'σπιτι', 'αυτοκινητο', 'λεωφορειο', 'υφασμα']
};
...
function inferSearchCategory(entry) {
    const explicit = normalizeFilterValue(entry.category);
    if (explicit && (explicit === 'general' || SEARCH_CATEGORY_RULES[explicit])) {
        return explicit;
    }
    const searchableText = normalizeSearchText([entry.cypriot, entry.turkish, entry.greek, entry.english, entry.definition].filter(Boolean).join(' '));
    for (const category of ['places', 'expressions', 'food', 'nature', 'people', 'objects']) {
        if (SEARCH_CATEGORY_RULES[category].some(term => searchableText.includes(normalizeSearchText(term)))) {
            return category;
        }
    }
    return 'general';
...
function inferSearchDialect(entry) {
    const explicit = normalizeFilterValue(entry.dialect);
    const aliases = {
        turkish: 'turkish-cypriot',
        'cypriot-turkish': 'turkish-cypriot',
        greek: 'greek-cypriot',
        'cypriot-greek': 'greek-cypriot',
        maronite: 'sanna',
        arabic: 'sanna'
    };
    if (explicit)
    ...
    if (entry.shared === true)
        return 'shared';
    const headword = String(entry.cypriot || '');
    const hasGreek = /\p{Script=Greek}/u.test(headword);
    const hasLatin = /\p{Script=Latin}/u.test(headword);
    if (hasGreek && hasLatin)
        return 'shared';
    return hasGreek ? 'greek-cypriot' : 'turkish-cypriot';
}

function getEntryAliases(entry) {
    const declared = Array.isArray(entry.aliases) ? entry.aliases : String(entry.aliases || '').split(/[|;,]/);
    const embedded = String(entry.cypriot || '').split(/[\/|;]/);
    return [...declared, ...embedded].map(normalizeSearchText).filter(Boolean);
}

function createWordPageSlug(word) {
    const charMap = {
        'α': 'a',
        'β': 'b',
        'γ': 'g',
    ...
    const safeName = String(word || '').toLowerCase().split('').map(char => charMap[char] !== undefined ? charMap[char] : char).join('').replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, '').replace(/-+/g, '-').replace(/^-+|-+$/g, '');
    return safeName || 'unnamed-word';
}

function buildSearchField(entry, field) {
    const rawValue = field === 'definition' ? (entry.definition || entry.notes || '') : entry[field];
    const value = normalizeSearchText(rawValue);
    const alternatives = field === 'cypriot' ? getEntryAliases(entry) : [];
    const allValues = [...new Set([value, ...alternatives].filter(Boolean))];
    const tokens = [...new Set(allValues.flatMap(getSearchTokens))];
    const transliterations = [...new Set(allValues.map(transliterateSearchText).filter(Boolean))];
    const transliteratedTokens = [...new Set(transliterations.flatMap(getSearchTokens))];
    return {
        value,
        alternatives,
        allValues,
...
const SEARCH_INDEX = dictionary.map( (entry, index) => ({
    entry,
    index,
    slug: createWordPageSlug(entry.cypriot),
    category: inferSearchCategory(entry),
    dialect: inferSearchDialect(entry),
    fields: SEARCH_FIELDS.reduce( (fields, field) => {
        fields[field] = buildSearchField(entry, field);
        return fields;
    }
    , {})
}));

function getAllowedTypoDistance(length) {
    if (length <= 1)
        return 0;
    if (length <= 4)
        return 1;
...
function fuzzyMatchScore(query, queryTokens, fieldData) {
    if (query.length < 2)
        return null;
    const candidates = [...fieldData.allValues, ...fieldData.tokens, ...fieldData.transliterations, ...fieldData.transliteratedTokens];
    let bestDistance = Infinity;
    candidates.forEach(candidate => {
        if (!candidate || Math.abs(candidate.length - query.length) > getAllowedTypoDistance(query.length))
            return;
        const distance = searchDistance(candidate, query);
        if (distance < bestDistance)
            bestDistance = distance;
    }
    );

    let phraseDistance = Infinity;
    if (queryTokens.length > 1 && fieldData.tokens.length) {
        let totalDistance = 0;
        let allTokensMatch = true;
        queryTokens.forEach(queryToken => {
            const tokenDistance = fieldData.tokens.reduce( (best, candidate) => {
                if (Math.abs(candidate.length - queryToken.length) > getAllowedTypoDistance(queryToken.length)) {
                    return best;
                }
                return Math.min(best, searchDistance(candidate, queryToken));
            }
            , Infinity);
            if (tokenDistance > getAllowedTypoDistance(queryToken.length)) {
        ...
        if (allTokensMatch)
            phraseDistance = totalDistance;
    }

    const allowedDistance = getAllowedTypoDistance(query.length);
    if (bestDistance > allowedDistance && phraseDistance === Infinity)
        return null;
    const effectiveDistance = Math.min(bestDistance, phraseDistance);
    return 2400 - (effectiveDistance * 180) - Math.max(0, fieldData.value.length - query.length);
}

function scoreSearchEntry(indexedEntry, query, language, options={}) {
    let bestMatch = null;
    const queryTokens = getSearchTokens(query);
    const transliteratedQuery = transliterateSearchText(query);
    const requestedField = options.searchIn && options.searchIn !== 'all' ? options.searchIn : null;
    SEARCH_FIELDS.filter(field => !requestedField || field === requestedField).forEach(field => {
        const fieldData = indexedEntry.fields[field];
        if (!fieldData.value)
            return;
        const priority = SEARCH_FIELD_PRIORITY[field] + (field === language ? 8 : 0);
        let matchType = null;
        let matchScore = 0;

        if (fieldData.value === query) {
            matchType = 'exact';
            matchScore = 10000;
        } else if (fieldData.alternatives.includes(query)) {
            matchType = 'alias';
            matchScore = 9500;
        } else if (fieldData.tokens.includes(query)) {
            matchType = 'word';
            matchScore = 9000;
        } else if (queryTokens.length > 1 && queryTokens.every(token => fieldData.tokens.includes(token))) {
            matchType = 'phrase';
            matchScore = 8200;
        } else if (fieldData.transliterations.includes(transliteratedQuery) && !fieldData.allValues.includes(query)) {
            matchType = 'transliteration';
            matchScore = 8000;
        } else if (fieldData.value.startsWith(query)) {
            matchType = 'prefix';
            matchScore = 7000;
        } else if (fieldData.tokens.some(token => token.startsWith(query))) {
            matchType = 'word-prefix';
            matchScore = 6500;
        } else if (fieldData.value.includes(query)) {
            matchType = 'contains';
            matchScore = 5000;
        } else if (fieldData.transliterations.some(value => value.includes(transliteratedQuery)) && !fieldData.allValues.some(value => value.includes(query))) {
            matchType = 'transliteration';
            matchScore = 4800;
        } else {
            const fuzzyScore = fuzzyMatchScore(transliteratedQuery || query, getSearchTokens(transliteratedQuery || query), fieldData);
            if (fuzzyScore !== null) {
                matchType = 'fuzzy';
        ...
        if (matchType) {
            const score = matchScore + priority;
...
function rankSearchResults(query, language, options={}) {
    query = normalizeSearchText(query);
    if (!query)
        return [];

    const ranked = SEARCH_INDEX.filter(indexedEntry => (!options.category || options.category === 'all' || indexedEntry.category === options.category) && (!options.dialect || options.dialect === 'all' || indexedEntry.dialect === options.dialect)).map(indexedEntry => {
        const match = scoreSearchEntry(indexedEntry, query, language, options);
        return match ? {
            ...indexedEntry,
            ...match
        } : null;
    }
    ).filter(Boolean).sort( (a, b) => b.score - a.score || a.entry.cypriot.localeCompare(b.entry.cypriot, undefined, {
        sensitivity: 'base'
    }) || a.index - b.index);

    // The generator merges entries that resolve to the same word-page slug.
    // Keep the strongest search hit for each page so duplicates do not crowd out results.
    const unique = [];
...
function getSearchMatchLabel(matchType, language, matchedField) {
    const labels = {
        exact: {
            english: 'Exact match',
            turkish: 'Tam eşleşme',
            greek: 'Ακριβής αντιστοίχιση'
        },
        alias: {
            english: 'Alternate spelling',
            turkish: 'Alternatif yazım',
            greek: 'Εναλλακτική γραφή'
        },
        word: {
            english: 'Word match',
            turkish: 'Kelime eşleşmesi',
            greek: 'Αντιστοίχιση λέξης'
        },
        phrase: {
            english: 'Phrase match',
            turkish: 'İfade eşleşmesi',
            greek: 'Αντιστοίχιση φράσης'
        },
        transliteration: {
            english: 'Transliterated match',
            turkish: 'Harf çevirisi eşleşmesi',
            greek: 'Αντιστοίχιση μεταγραφής'
        },
        prefix: {
            english: 'Prefix match',
            turkish: 'Başlangıç eşleşmesi',
            greek: 'Αντιστοίχιση αρχής'
        },
        'word-prefix': {
            english: 'Word-prefix match',
            turkish: 'Kelime başlangıcı',
            greek: 'Αρχή λέξης'
        },
        contains: {
            english: 'Contains match',
            turkish: 'İçeren eşleşme',
            greek: 'Αντιστοίχιση που περιέχει'
        },
        fuzzy: {
            english: 'Close match',
            turkish: 'Yakın eşleşme',
            greek: 'Κοντινή αντιστοίχιση'
        }
    };
    const languageLabels = labels[matchType] || labels.contains;
    const fieldLabels = {
        cypriot: {
            english: 'Cypriot',
            turkish: 'Kıbrısça',
    ...
    const matchLabel = languageLabels[language] || languageLabels.english;
    const fieldLabel = fieldLabels[matchedField] ? (fieldLabels[matchedField][language] || fieldLabels[matchedField].english) : '';
    return fieldLabel ? `${matchLabel} · ${fieldLabel}` : matchLabel;
}

// Make the current dictionary available to pages that share this data source.
window.cylingoDictionary = dictionary;
window.CylingoSearch = {
    rankSearchResults,
    getSearchMatchLabel,
    getIndexedEntries: () => SEARCH_INDEX.slice()
};

// Event listener for search input changes (only present on search-enabled pages)
...
// Function to update search results based on current input and language
function updateSearch() {
    const searchElement = document.getElementById('searchInput');
    const languageElement = document.getElementById('language');
    if (!searchElement || !languageElement)
        return;

    const input = normalizeSearchText(searchElement.value);
    ...
    const language = languageElement.value.toLowerCase();
    const options = {
        searchIn: document.getElementById('searchField')?.value || 'all',
        category: document.getElementById('categoryFilter')?.value || 'all',
        dialect: document.getElementById('dialectFilter')?.value || 'all'
    };

    if (input === '') {
        displayRandomWord();
        // Display random word when input is empty
    } else {
        const results = rankSearchResults(input, language, options);
        displaySuggestions(results, language, input);
        if (window.CylingoAnalytics) {
            window.CylingoAnalytics.trackSearch({
                language,
                search_field: options.searchIn,
                category: options.category,
                dialect: options.dialect,
                input_length: input.length,
                result_count: results.length,
                direct_result_count: results.filter(result => result.matchType !== 'fuzzy').length,
                has_exact_match: results.some(result => result.matchType === 'exact'),
                has_close_match: results.some(result => result.matchType === 'fuzzy')
            });
            const knownMatch = results.find(result => result.matchType === 'exact' || result.matchType === 'word');
            if (knownMatch) {
                window.CylingoAnalytics.trackKnownWordSearch({
                    source: 'dictionary_entry',
                    word_slug: knownMatch.slug,
                    language,
                    match_type: knownMatch.matchType
                });
            }
        }
    }
...
// Add modal HTML to the body only if we're on a page that needs it
document.addEventListener('DOMContentLoaded', function() {
    // Only add modal if there's a search functionality (dictionary pages)
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div class="share-modal">
                ...
                <div class="modal-content">
                    <h3>Share this word</h3>
                    <p id="share-word-text"></p>
                    <div class="share-buttons">
                        <button class="twitter-share">Twitter</button>
                        <button class="facebook-share">Facebook</button>
                        <button class="whatsapp-share">WhatsApp</button>
                    </div>
                </div>
            </div>
        `);

        // Set up modal event listeners after adding HTML
        setupModalEventListeners();
    }
});

function showShareModal(word, translation, language) {
    const modal = document.querySelector('.share-modal');
    const overlay = document.querySelector('.modal-overlay');
    const wordText = document.getElementById('share-word-text');
    const languageElement = document.getElementById('language');

    if (!modal || !overlay || !wordText) {
        console.log('Modal elements not found');
        return;
    }

    const selectedLanguage = languageElement ? languageElement.value.toLowerCase() : 'english';
    const entry = dictionary.find(item => item.cypriot === word);

    const shareTranslation = entry && entry[selectedLanguage] ? entry[selectedLanguage] : translation;
    wordText.textContent = `${word} (${selectedLanguage}: ${shareTranslation})`;

    const shareText = `🇨🇾 Check out this Cypriot word!\n\n${word}\n${selectedLanguage}: ${shareTranslation}\n\nLearn more Cypriot words at Cylingo Dictionary:`;
    const shareUrl = "https://cylingo.com";
    ...
    if (whatsappShare) {
        whatsappShare.onclick = () => {
            const fullText = `🇨🇾 Check out this Cypriot word!\n\n${word}\n${language}: ${translation}\n\nLearn more Cypriot words at:\nhttps://cylingo.com`;
...
function displaySuggestions(suggestions, language, query='') {
    ...
    if (suggestions.length === 0) {
        ...
        emptyState.innerHTML = '<span class="suggestion-empty-icon" aria-hidden="true">⌕</span>';
        const emptyCopy = document.createElement('span');
        const emptyTitle = document.createElement('strong');
        emptyTitle.textContent = `No matches for “${query}”`;
        const emptyHint = document.createElement('small');
        emptyHint.textContent = 'Try another spelling or search in a different language.';
        emptyCopy.appendChild(emptyTitle);
        emptyCopy.appendChild(emptyHint);
    ...
    const directResults = suggestions.filter(result => result.matchType !== 'fuzzy').slice(0, 10);
    const similarResults = suggestions.filter(result => result.matchType === 'fuzzy').slice(0, 4);

    const resultsToolbar = document.createElement('div');
    resultsToolbar.className = 'suggestions-toolbar';
    const resultsTitle = document.createElement('strong');
    resultsTitle.textContent = 'Dictionary results';
    const resultsCount = document.createElement('span');
    resultsCount.textContent = `${directResults.length + similarResults.length} found`;
    resultsToolbar.appendChild(resultsTitle);
    resultsToolbar.appendChild(resultsCount);
    suggestionsContainer.appendChild(resultsToolbar);

    function addHeader(label) {
        const didYouMeanHeader = document.createElement('div');
        didYouMeanHeader.className = 'suggestion-header';
        didYouMeanHeader.textContent = label;
        suggestionsContainer.appendChild(didYouMeanHeader);
    }

    function appendResult(result, position) {
        const entry = result.entry;
        const translation = entry[language] || "Translation not available";
        const div = document.createElement('div');
        div.className = result.matchType === 'fuzzy' ? 'suggestion-item suggestion-similar' : 'suggestion-item';
        div.dataset.cypriotWord = entry.cypriot;
        div.dataset.searchLanguage = language;
        div.dataset.matchType = result.matchType;
        div.dataset.resultPosition = String(position);

        const wordLink = document.createElement('a');
        wordLink.className = 'suggestion-link';
        wordLink.href = `words/${result.slug}.html`;
        wordLink.setAttribute('aria-label', `Open ${entry.cypriot}, ${translation}`);

        const resultIcon = document.createElement('span');
        resultIcon.className = 'suggestion-icon';
        resultIcon.setAttribute('aria-hidden', 'true');
        resultIcon.textContent = entry.cypriot.trim().charAt(0).toUpperCase();

        const wordInfo = document.createElement('span');
        wordInfo.className = 'suggestion-copy';
        const headword = document.createElement('strong');
        headword.className = 'suggestion-word';
        headword.textContent = entry.cypriot;
        const meaning = document.createElement('span');
        meaning.className = 'suggestion-meaning';
        meaning.textContent = translation;
        const matchLabel = document.createElement('small');
        matchLabel.className = 'suggestion-match';
        matchLabel.textContent = getSearchMatchLabel(result.matchType, language, result.matchedField);
        wordInfo.appendChild(headword);
        wordInfo.appendChild(meaning);
        wordInfo.appendChild(matchLabel);
        wordLink.appendChild(resultIcon);
        wordLink.appendChild(wordInfo);
        const resultArrow = document.createElement('span');
        resultArrow.className = 'suggestion-arrow';
        resultArrow.setAttribute('aria-hidden', 'true');
        resultArrow.textContent = '→';
        wordLink.appendChild(resultArrow);

        const shareButton = document.createElement('button');
        shareButton.type = 'button';
        shareButton.className = 'share-button';
        shareButton.innerHTML = '<i class="fas fa-share-alt"></i> Share';
        shareButton.setAttribute('aria-label', `Share ${entry.cypriot}`);
        shareButton.onclick = event => {
            event.preventDefault();
            event.stopPropagation();
            if (window.CylingoAnalytics) {
                window.CylingoAnalytics.track('word_shared', {
                    channel: navigator.share ? 'native' : 'clipboard',
                    source: 'dictionary_search',
                    language
                });
            }
            const text = `Check out this Cypriot word:\n${entry.cypriot}\n${capitalizeFirstLetter(language)}: ${translation}\n\nLearn more Cypriot words at cylingo.org!`;
            if (navigator.share) {
                navigator.share({
                    title: 'Cypriot Word',
                    text: text
                ...
                navigator.clipboard.writeText(text).then( () => alert('Word copied to clipboard!')).catch(console.error);
            }
        }
        ;

        div.appendChild(wordLink);
    ...
    if (directResults.length) {
        if (similarResults.length)
            addHeader('Results');
        directResults.forEach( (result, index) => appendResult(result, index + 1));
    }
    if (similarResults.length) {
        addHeader('Did you mean?');
        similarResults.forEach( (result, index) => appendResult(result, directResults.length + index + 1));
    }
}

// Function to display a random word when input is empty
function displayRandomWord() {
    ...
    div.textContent = `Did you know this word: ${randomEntry.cypriot}?`;
    suggestionsContainer.appendChild(div);
...
// Function to change the language and update suggestions
function changeLanguage() {
    // Use the updateSearch function to refresh results with the new language
    updateSearch();
}

// Add event listener for language change
document.addEventListener('DOMContentLoaded', function() {
    ['language', 'searchField', 'categoryFilter', 'dialectFilter'].forEach(id => {
...
function shareWord() {
    const selectedWord = document.querySelector('.suggestion-item[data-cypriot-word]');
    if (selectedWord) {
        const entry = dictionary.find(item => item.cypriot === selectedWord.dataset.cypriotWord);

        if (entry) {
            const text = `Check out this Cypriot word: ${entry.cypriot}\nEnglish: ${entry.english}\n\nLearn more Cypriot words at Cylingo!`;
...
// Social media sharing functionality (modified to use english translation)
function shareOnTwitter(word, translation) {
    const languageElement = document.getElementById('language');
    const selectedLanguage = languageElement ? languageElement.value.toLowerCase() : 'english';
    const text = `Check out this Cypriot word: ${word} - ${translation} (${selectedLanguage})`;
    const url = 'https://twitter.com/intent/tweet?text=' + encodeURIComponent(text);
    window.open(url, '_blank');
}

function shareOnFacebook(word, translation) {
    const languageElement = document.getElementById('language');
    const selectedLanguage = languageElement ? languageElement.value.toLowerCase() : 'english';
    const text = `Check out this Cypriot word: ${word} - ${translation} (${selectedLanguage})`;
...
function shareOnWhatsApp(word, translation) {
    const languageElement = document.getElementById('language');
    const selectedLanguage = languageElement ? languageElement.value.toLowerCase() : 'english';
    const text = `Check out this Cypriot word: ${word} - ${translation} (${selectedLanguage})`;
    ...
    if (twitterBtn) {
        twitterBtn.addEventListener('click', () => {
            ...
            if (selectedWord) {
                const entry = dictionary.find(item => item.cypriot === selectedWord.dataset.cypriotWord);
                if (entry)
                    shareOnTwitter(entry.cypriot, entry.english);
            }
        }
        );
    }
    ...
    if (facebookBtn) {
        facebookBtn.addEventListener('click', () => {
            ...
            if (selectedWord) {
                const entry = dictionary.find(item => item.cypriot === selectedWord.dataset.cypriotWord);
                if (entry)
                    shareOnFacebook(entry.cypriot, entry.english);
            }
        }
        );
    }
    ...
    if (whatsappBtn) {
        whatsappBtn.addEventListener('click', () => {
            ...
            if (selectedWord) {
                const entry = dictionary.find(item => item.cypriot === selectedWord.dataset.cypriotWord);
                if (entry)
                    shareOnWhatsApp(entry.cypriot, entry.english);
            }
        }
        );
    }
...
// An accidental duplicate copy of this file was appended below. Keep it inert
// until the generated dictionary source is compacted in a dedicated cleanup.
if (false) {
    const dictionary = [{
        cypriot: "τραφικέϊτο",
        turkish: "Araç sinyali; yön göstergesi",
